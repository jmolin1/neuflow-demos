clc; addpath('tools'); close all; warning off;
...
for ii = idx % for every sequence
    ...
    for i = 1 : length(Tracker)
        if exist('InputPath')
            if length(Tracker) > 1
                filename = [InputPath filesep sequence filesep Tracker{i} '.txt']
            else
                filename = [InputPath filesep sequence '.txt']
            end
        else
...
fout = fopen([ResultPath filesep 'results.txt'],'w');
...
disp(['Results saved to ' ResultPath filesep 'results.txt']);