...
local function getCoordinates(args)
   ...
   local ker_width = args.ker_width
   local ker_height = args.ker_height
   ...
   local dim_width = math.floor(dim_width_orig*scales[1]) + ker_width - 1
   local dim_height = math.floor(dim_height_orig*scales[1]) + ker_height - 1
   ...
   for i=2,#scales,1 do
      ...
      dim_width = math.floor(dim_width_orig*scales[i]) + ker_width - 1
      dim_height = math.floor(dim_height_orig*scales[i]) + ker_height - 1
...
function PyramidPacker:__init(network, scales, pad)
   parent.__init(self)
   -- vars
...
function PyramidPacker:forward(input)
   ...
   if ((input:size(3) ~= self.dim_width) or (input:size(2) ~= self.dim_height)) then
      ...
      self.coordinates, self.max_width, self.max_height =
         getCoordinates({dim_width = self.dim_width,
                         dim_height = self.dim_height,
                         scales = self.scales,
                         step_width = self.step_width,
                         step_height = self.step_height,
                         ker_width = self.ker_width,
                         ker_height = self.ker_height,
                         pad = self.pad})
   end
   ...
   if  self.pad then  --not
      ...
      for i = 1,#self.scales do
         ...
         image.scale(input, temp, 'simple')
      end
   else
      -- padding
      for i = 1,#self.scales do
         local temp = self.output:narrow(3,self.coordinates[i][1]+math.ceil(self.ker_width/2),
                                         self.coordinates[i][5]-self.ker_width+1)
         temp = temp:narrow(2,self.coordinates[i][2]+math.ceil(self.ker_height/2),
                            self.coordinates[i][6]-self.ker_height+1)
         image.scale(input, temp, 'simple')
      end
   end
