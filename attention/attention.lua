...
op:option {
   '-c', '--camera', action = 'store', dest = 'camidx',
...
function process()
   ...
   if (last_fps ~= -1) then
      ...
      str = string.format('speedup = %f ', speedup)
      -- disp line:
      win:moveto(x,y);
      win:show(str)
   end
